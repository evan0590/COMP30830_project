	...
	if ( !document.getElementById('end').value | !document.getElementById('start').value){
		...
		if ( parseInt(distanceResults[2])<10 && parseInt(distanceResults[5])<10){
			for (i = 0; i < stations1.length; i++) {  
				if ( stations1[i][0] == distanceResults[0]){
					var startLat = stations1[i][2]
					var startLong = stations1[i][3]	
					waypts.push({
...
function distance(lat1, lon1, lat2, lon2, unit) {
	...
	if ((startLat == endLat) && (startLon == endLon)) {
		...
		for (i = 0; i < stations1.length; i++) {  
			...
			var radlatStations = Math.PI * stations1[i][2]/180;
			var thetaStart = startLon-stations1[i][3];
